...
def test_failed_notification_is_attempted_twice_then_isolated(tmp_path: Path) -> None:
    container = bootstrap(
        Settings(paths=AppPaths.under(tmp_path / "data"), minimum_free_disk_bytes=0)
    )
    job = container.workflow.create_job("P", 35.7, 51.4, 100, "MAIN", 1)
    ...
    def failing(_method: str, _payload: dict[str, str]) -> dict[str, object]:
        nonlocal calls
        calls += 1
        raise OSError("sensitive upstream detail")

    notifier = TelegramNotifier(container.database, "secret-token", "123", failing)
    assert notifier.dispatch_pending().failed == 1
    assert notifier.dispatch_pending().failed == 1
    assert notifier.dispatch_pending().failed == 0
    assert calls == 2
    with container.database.connect() as connection:
        event = connection.execute(
            "SELECT notify_attempts, last_notify_error FROM intervention_events"
        ).fetchone()
    assert event["notify_attempts"] == 2
    assert event["last_notify_error"] == "telegram_delivery_failed"
    assert "sensitive" not in str(event)


def test_message_uses_configured_chat_without_exposing_token(tmp_path: Path) -> None:
    container = bootstrap(
        Settings(paths=AppPaths.under(tmp_path / "data"), minimum_free_disk_bytes=0)
    )
    calls: list[tuple[str, dict[str, str]]] = []
    ...
    notifier = TelegramNotifier(container.database, "secret-token", "456", fake)
    assert notifier.send_test_message()
    assert calls == [("sendMessage", {"chat_id": "456", "text":
                                      "Near Project Environment: test OK"})]
    assert "secret-token" not in str(calls)
